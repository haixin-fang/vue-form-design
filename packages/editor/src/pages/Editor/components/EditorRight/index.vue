...
export default defineComponent({
  setup() {
    ...
    const controlItems = computed(() => store.getters.getControlItems);
    const curControl = computed(() => store.state.form.curControl);
    const save = computed(() => store.state.form.save);
    ...
    // 预览或保存时验证所有表单是否输入正确
    const preview = computed(() => store.state.form.preview);
    const allFormList = computed(() => store.getters.getAllFormList);
    ...
    const checkValidates = async (formSave?: boolean) => {
      const curControlIndex = store.state.form.currentIndex;
