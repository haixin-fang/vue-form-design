<template>
  <div class="conditionSelect" :class="result.type">
    ...
    <div
      class="control"
      v-show="
        result.result && result.result.length > 0 && result.type !== 'data'
      "
    >
      <span
        class="iconfont icon-jiahao"
        v-if="!result.control"
        @click="handleControl(true)"
      ></span>
    ...
    <Transition>
      <div
        v-show="result.type && result.type !== 'data' && result.control"
        class="moreCondition"
      >
        <div
        ...
        <div
          class="line"
          :class="result.type"
          v-if="result.result && result.result.length >= 2"
        ></div>
      </div>
    </Transition>
